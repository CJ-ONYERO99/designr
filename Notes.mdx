...
- avoid middleware.ts if possible because it runs only on edge but i want to be able to deploy my app to any server
- Route Groups:
...
- why i use canvasContainerRef
  - Canvas resizing is difficult hence i am using a div as canvasContainerRef and i will use a resize observer to see if canvasContainerRef is resized and use the width and height from there to set width and height of canvas
...
### 26. Underline, linethrough, uppercase
...
- Does not use file and folder structure like NextJS
  ...
  - i will learn more about RPC routes
...
- https://hono.dev/docs/guides/validation
  - hono has thin minimal validation. so use a third party library for validation like zod
...
### 32. Use Bytescale to upload files for my project
...
- Note: NextAuth v4 is now called as Auth.js which is v5 so make sure i am on the correct website
- https://authjs.dev/ ✅ v5 - i will be using this
...
- For google and github i have used db session strategy but for credentials i will use jwt strategy
...
- Note: Better to avoid credentials provider and stick to OAuth providers, magic link etc. And if that's the case, then i can use the db session strategy for all providers and get rid of jwt strategy

## Day 9

### 48. Adding auth for hono API

- create a test.ts route to make it a protected route
...
- `stripe listen --forward-to localhost:3000/api/subscriptions/webhook` - please do not close this for testing purposes
...
- Note: I am not using the Image tag from Next.js because I am currently using the free tier which has a limit on image optimization. If i have a paid plan then i can use the Image tag from Next.js which is recommended
